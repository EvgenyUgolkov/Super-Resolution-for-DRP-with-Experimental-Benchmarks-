...
def main_worker(gpu, ngpus_per_node, args):
    ...
    def plot_intermediate(g_input_plot, output_plot):
        ...
        for i in range(2):
            for j in range(1):
                ...
                axarr[i].imshow(images[i][j, middle, :, :], cmap='gray', vmin=0, vmax=5)
                axarr[i].set_xticks([0, length_im-1])
                axarr[i].set_yticks([0, length_im-1])
        plt.suptitle('Stage 4 slides comparison')
    ...
    class D_0(nn.Module):
        def __init__(self):
            ...
            self.from_0 = nn.Conv2d(6, 512, 1, 1, 0)
    ...
    class D_1(nn.Module):
        def __init__(self):
            ...
            self.d0 = D_0()
            self.d0.load_state_dict(torch.load('D_0_pretrained_l_6.pth'))
            self.from_1 = nn.Conv2d(6, 256, 1, 1, 0)
    ...
    class D_2(nn.Module):
        def __init__(self):
            ...
            self.d1 = D_1()
            self.d1.load_state_dict(torch.load('D_1_pretrained_l_6.pth'))
            self.from_2 = nn.Conv2d(6, 128, 1, 1, 0)
    ...
    class D_3(nn.Module):
        def __init__(self):
            ...
            self.d2 = D_2()
            self.d2.load_state_dict(torch.load('D_2_pretrained_l_6.pth'))
            self.from_3 = nn.Conv2d(6, 64, 1, 1, 0)
    ...
    class D_4(nn.Module):
        def __init__(self):
            ...
            self.d3 = D_3()
            self.d3.load_state_dict(torch.load('D_3_pretrained_l_6.pth'))
            # self.from_4 = nn.Conv2d(5, 32, 1, 1, 0)
            # self.conv7 = nn.Conv2d(32, 64, 3, 2, 1)
            self.conv7 = nn.Conv2d(6, 64, 3, 2, 1)
            self.alpha = 0
                ...
                if (i % g_update) == 0:
                    ...
                    with autocast():
                        ...
                        for k in range(math.comb(n_dims, 2)):
                            ...
                            if stage < 2: # in x32 it was < 3 because we started from 16
    ...
    scale_f_4 = 8
    scale_f_5 = 16
    to_low_idx_interm = torch.LongTensor([1,2,3,4]).to(device)
    ...
    chin_out = 6  # N of minerals + 1 channel for the mixed. Here we have pore + quartz + feltspar + clay + mixed -> 5
    ...
    torch.save(netD.module.state_dict(), 'D_0_pretrained_l_6.pth')
    ...
    torch.cuda.empty_cache()
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
    torch.save(netD.module.state_dict(), 'D_1_pretrained_l_6.pth')
    ...
    torch.cuda.empty_cache()
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
    torch.save(netD.module.state_dict(), 'D_2_pretrained_l_6.pth')
    ...
    torch.cuda.empty_cache()
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
    torch.save(netD.module.state_dict(), 'D_3_pretrained_l_6.pth')
