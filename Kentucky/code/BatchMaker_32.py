...
HIGH_L_3D = [16, 32, 64, 128, 256, 512]  # length of high resolution 2d - to make it for different stages
...
class BatchMaker:
    ...
    def __init__(self, stage, device, to_low_idx=None, path=NMC_PATH, sf=4, dims=3,
        ...
        if stack and not low_res and stage == 5:  # it is the high-res training data            
        ...
        if low_res: # this one is for the 3D LR sampling. Here it should be constant all the time, for all of the stages -> fix it with 5
            self.high_l = int(HIGH_L_3D[5]/self.scale_factor)
        if self.dims == 2:
