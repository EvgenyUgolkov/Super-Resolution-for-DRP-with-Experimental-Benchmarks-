...
# Number of GPUs for the DDP process
ngpu = 2
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    def plot_intermediate(g_input_plot, output_plot):
        ...
        for i in range(2):
            for j in range(1):
                ...
                axarr[i].imshow(images[i][j, middle, :, :], cmap='gray', vmin=0, vmax=6)
    ...
    class G_1(torch.nn.Module):
        ...
        def forward(self, x, memorized_0, stage):
            ...
            if stage == 1:
                sftmx_dense = back_to_dense_1(sftmx, memorized_0, self.factors[1], self.steps[4], (1, self.chin_out, 32, 32, 32))
            else:
                sftmx_dense = None
    ...
    class G_2(torch.nn.Module):
        def __init__(self, chin_out, channels, factors, steps):
            ...
            self.classification_2 = ME.MinkowskiConvolution(self.channels[4], self.chin_out, kernel_size=1, bias=True, dimension=3)
        ...
        def forward(self, x, memorized_0, memorized_1, stage):
            ...
            if stage == 2:
                sftmx_dense = back_to_dense_2(sftmx, memorized_1, memorized_0, self.factors[2], self.steps[4], (1, self.chin_out, 64, 64, 64))
            else:
                sftmx_dense = None
    ...
    class G_3(torch.nn.Module):
        def __init__(self, chin_out, channels, factors, steps):
            ...
            self.classification_3 = ME.MinkowskiConvolution(self.channels[3], self.chin_out, kernel_size=1, bias=True, dimension=3)
        ...
        def forward(self, x, memorized_0, memorized_1, memorized_2, stage):
            ...
            if stage == 3:
                sftmx_dense = back_to_dense_3(sftmx, memorized_2, memorized_1, memorized_0, self.factors[3], self.steps[4], (1, self.chin_out, 128, 128, 128))
            else:
                sftmx_dense = None
    ...
    class G_4(torch.nn.Module):
        def __init__(self, chin_out, channels, factors, steps):
            ...
            self.classification_4 = ME.MinkowskiConvolution(self.channels[2], self.chin_out, kernel_size=1, bias=True, dimension=3)
            
            self.softmax = ME.MinkowskiSoftmax(dim=1)
            self.pruning = ME.MinkowskiPruning()
    
        def forward(self, x, memorized_0, memorized_1, memorized_2, memorized_3, stage):
            ...
            mask = torch.argmax(sftmx.F, dim=1) == (sftmx.F.size(1) - 1)
            x = self.pruning(x, mask)
            memorized_4 = self.pruning(sftmx, ~mask)
            
            if stage == 4:
                sftmx_dense = back_to_dense_4(sftmx, memorized_3, memorized_2, memorized_1, memorized_0, self.factors[4], self.steps[4], 
                                              (1, self.chin_out, 256, 256, 256))
            else:
                sftmx_dense = None
    
            return x, memorized_4, sftmx_dense
    ###################################################################################################################################
    class G_5(torch.nn.Module):
        def __init__(self, chin_out, channels, factors, steps):
            super(G_5, self).__init__()
            self.chin_out = chin_out
            self.channels = channels
            self.factors = factors
            self.steps = steps
            ...
            self.classification_5 = ME.MinkowskiConvolution(self.channels[1], self.chin_out, kernel_size=1, bias=True, dimension=3)
            
            self.softmax = ME.MinkowskiSoftmax(dim=1)
            self.pruning = ME.MinkowskiPruning()
    
        def forward(self, x, memorized_0, memorized_1, memorized_2, memorized_3, memorized_4):
            
            # Upconv
            x = self.block5(x)
    
            # Predict their values
            sftmx = self.softmax(self.classification_5(x))
            
            # Transfer to the final dense tensor
            sftmx_dense = back_to_dense_5(sftmx, memorized_4, memorized_3, memorized_2, memorized_1, memorized_0, self.steps[4], 
                                          (1, self.chin_out, 512, 512, 512))
    
            return sftmx_dense
    ###################################################################################################################################
    class ProgressiveGenerator(nn.Module):
        def __init__(self, stage0, stage1, stage2, stage3, stage4, stage5):
        ...
        def forward(self, x):
            ...
            if self.current_stage >= 4:
                ...
                x, memorized_4, sftmx_dense = self.stage4(x, memorized_0, memorized_1, memorized_2, memorized_3, self.current_stage)
                
            if self.current_stage == 5:
                
                sftmx_dense = self.stage5(x, memorized_0, memorized_1, memorized_2, memorized_3, memorized_4)
                
            return sftmx_dense
                
    ###################################################################################################################################
    class D_0(nn.Module):
        def __init__(self, chin_out):
            super(D_0, self).__init__()
            self.chin_out = chin_out
            self.from_0 = nn.Conv2d(self.chin_out, 512, 1, 1, 0)
        ...
        def forward(self, x):
            ...
            return x
    ###################################################################################################################################
    class D_01(nn.Module):
        def __init__(self, chin_out):
            super(D_01, self).__init__()
            self.chin_out = chin_out
            self.d0 = D_0(self.chin_out)
            self.d0.load_state_dict(torch.load('D_0_pretrained_x32_test_37.pth'))
            self.from_01 = nn.Conv2d(self.chin_out, 512, 1, 1, 0)
    ...
    class D_1(nn.Module):
        def __init__(self, chin_out):
            ...
            self.chin_out = chin_out
            self.d01 = D_01(self.chin_out)
            self.d01.load_state_dict(torch.load('D_01_pretrained_x32_test_37.pth'))
            self.from_1 = nn.Conv2d(self.chin_out, 256, 1, 1, 0)
            self.conv4 = nn.Conv2d(256, 512, 3, 2, 1)
            self.alpha = 0
    
        def forward_layers(self, x):
            x = nn.ReLU()(self.conv4(x))
            x = self.d01.forward_layers(x)
            return x  
        ...
        def forward(self, x):
            if self.alpha < 1:
                ...
                new_y = int(x.size()[3]/2)
                x_pred = F.interpolate(x, size=(new_x, new_y), mode='nearest')
                x_pred = nn.ReLU()(self.d01.from_01(x_pred))
                # Current stage
    ...
    class D_2(nn.Module):
        def __init__(self, chin_out):
            ...
            self.chin_out = chin_out
            self.d1 = D_1(self.chin_out)
            self.d1.load_state_dict(torch.load('D_1_pretrained_x32_test_37.pth'))
            self.from_2 = nn.Conv2d(self.chin_out, 128, 1, 1, 0)
    ...
    class D_3(nn.Module):
        def __init__(self, chin_out):
            ...
            self.chin_out = chin_out
            self.d2 = D_2(self.chin_out)
            self.d2.load_state_dict(torch.load('D_2_pretrained_x32_test_37.pth'))
            self.from_3 = nn.Conv2d(self.chin_out, 64, 1, 1, 0)
    ...
    class D_4(nn.Module):
        def __init__(self, chin_out):
            ...
            self.chin_out = chin_out
            self.d3 = D_3(self.chin_out)
            self.d3.load_state_dict(torch.load('D_3_pretrained_x32_test_37.pth'))
            # self.from_4 = nn.Conv2d(5, 32, 1, 1, 0)
            # self.conv7 = nn.Conv2d(32, 64, 3, 2, 1)
            self.conv7 = nn.Conv2d(self.chin_out, 64, 3, 2, 1)
            self.alpha = 0
    ...
    ###################################################################################################################################
    def train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage):
                        ...
                        for k in range(math.comb(n_dims, 2)):
                            ...
                            if stage < 3:
                                ...
                                if pix_loss.item() > 0.05:
                                    g_cost += -fake_output + pix_distance_stage * pix_loss
                                else:
                                    g_cost += -fake_output
                            else: 
                                g_cost += -fake_output
    ...
    scale_f_5 = 16
    scale_f_6 = 32
    to_low_idx_interm = torch.LongTensor([1,2,3,4,5]).to(device)
    ...
    down_sample_object_6 = LearnTools.DownSample_2(squash, n_dims, to_low_idx_interm, scale_f_6, device, super_sampling, separator).to(device)
    down_sample_objects = [down_sample_object_1, down_sample_object_2, down_sample_object_3, down_sample_object_4, down_sample_object_5, down_sample_object_6]  
    
    # Create the generator
    chin_out = 7  # N of minerals + 1 channel for the mixed. Here we have pore + quartz + feltspar + clay + mixed -> 5
    factors = [32, 16, 8, 4, 2]  # we have 6 because input is 32 and this is 2^5 - we go 5 steps to 1x1x1 + 1 for the initial size for the res conv
    steps = [32, 16, 8, 4, 2]  # these steps are the distance between the nodes for different levels of upsampling. For x8 we have 3 levels of upsampling
    ...
    G_stage5 = G_5(chin_out, channels, factors, steps).to(device)
    
    model_G = ProgressiveGenerator(G_stage0, G_stage1, G_stage2, G_stage3, G_stage4, G_stage5).to(device)
    ...
    torch.cuda.empty_cache()
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
    torch.save(netD.module.state_dict(), 'D_0_pretrained_x32_test_37.pth')
    
    # Stage 1
    print('Running Stage 1')
    stage = 1
    num_epochs = 248
    pix_distance_stage = 10
    model_G.module.set_stage(1)
    optimizerG = optim.AdamW(filter(lambda p: p.requires_grad, model_G.parameters()), lr=lr, betas=(beta1, 0.999), eps=1e-04, weight_decay=0.05)               
    netD = D_01(chin_out).to(device)
    netD = torch.nn.parallel.DistributedDataParallel(netD, device_ids=[args.gpu], find_unused_parameters=True)
    optimizerD = optim.AdamW(netD.parameters(), lr=lr, betas=(beta1, 0.999), eps=1e-04, weight_decay=0.05)
    BM_D = BatchMaker(stage, device, path=D_images[1], sf=scale_f, dims=n_dims, stack=True, low_res=False, rot_and_mir=True)
    torch.cuda.empty_cache()
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
    torch.save(netD.module.state_dict(), 'D_01_pretrained_x32_test_37.pth')
    ...
    torch.cuda.empty_cache()
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
    torch.save(netD.module.state_dict(), 'D_3_pretrained_x32_test_37.pth')
    ...
    num_epochs = 100
    pix_distance_stage = 0
    model_G.module.set_stage(5)
    optimizerG = optim.AdamW(filter(lambda p: p.requires_grad, model_G.parameters()), lr=lr2, betas=(beta1, 0.999), eps=1e-04, weight_decay=0.05)              
    netD = D_4(chin_out).to(device)
    netD = torch.nn.parallel.DistributedDataParallel(netD, device_ids=[args.gpu], find_unused_parameters=True)
    optimizerD = optim.AdamW(netD.parameters(), lr=lr2, betas=(beta1, 0.999), eps=1e-04, weight_decay=0.05)
    BM_D = BatchMaker(stage, device, path=D_images[5], sf=scale_f, dims=n_dims, stack=True, low_res=False, rot_and_mir=True)
    ...
    torch.cuda.empty_cache()
    print('Starting actually training Stage 5')
    train_progressive_stage(num_epochs, model_G, netD, optimizerD, BM_D, stage, pix_distance_stage)
